...
function VendorNotifications() {
  ...
  const fetchSeenData = async () => {
    try {
      ...
      setSeenNotifications(response.data.results)
    } catch (error) {
      console.error("Error fetching data:", error)
    }
  }
  ...
  const handleNotificationSeenStatus = async (notifId) => {
    try {
      const response = await axios.get(
        `vendor-notifications-mark-as-seen/${userData?.vendor_id}/${notifId}/`
      )
              ...
              <div className="col-xl-4 col-lg-6 mb-2">
                <div className="card card-inverse card-success">
                  <div className="card-block bg-primary p-3">
                    ...
                    <h1 className="display-1">{notificationStats.all_notif}</h1>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <tbody>
                    {notifications?.map((notif, index) => (
                      <tr key={index}>
                        <td>
                          {notif.order !== null && (
                            <p>New Order {notif?.order?.oid}</p>
                          )}
                        </td>
                        <td>
                          {notif.order_item !== null && (
                            <p>
                              You've got a new order for{" "}
                              <b>{notif?.order_item?.product?.title}</b>
                            </p>
                          )}
                        </td>
                        <td>
                        ...
                        <td>{moment(notif.date).format("MMM/DD/YYYY")}</td>
                        <td>
                          {notif.seen === true ? (
                            ...
                            <button
                              onClick={() =>
                                handleNotificationSeenStatus(notif.id)
                              }
                        ...
                        <div className="modal-content">
                          ...
                          <div className="modal-body">
                            <table className="table">
                              ...
                              <tbody>
                                {seenNotification?.map((notif, index) => (
                                  <tr key={index}>
                                    <td>
                                      {notif.order !== null && (
                                    ...
                                    <td>
                                      {notif.order_item !== null && (
                                        <p>
                                          You've got a new order for{" "}
                                          <b>
                                            {notif?.order_item?.product?.title}
                                          </b>
                                        </p>
                                      )}
                                    </td>
                                    ...
                                    <td>
                                      {moment(notif.date).format("MMM/DD/YYYY")}
                                    </td>
                                  </tr>
                                ))}
